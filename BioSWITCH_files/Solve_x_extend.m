function [k_extend, x_extend, set_idx, Str_sol] = Solve_x_extend(ESP,COM,KNS,network_file)
...
if selec == 0
    ii=1;
    while ii<=ncomb && isempty(Str_sol)
        ii
        x_extend(set(ii,:));
        S{ii} = solve(H_s==0,x_extend(set(ii,:)));
        S{ii}
        
        EmptyIndex = structfun(@isempty,S{ii});
